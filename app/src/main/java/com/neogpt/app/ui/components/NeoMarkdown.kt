...
/**
 * Lightweight Markdown renderer designed for streaming. It never replays an
 * entrance animation for every incoming token, which prevents the flashing/glitch
 * effect that was visible while an answer was being streamed.
 */
@Composable
fun NeoMarkdown(
    markdown: String,
...
private fun NeoMarkdownLine(line: String, textScale: Float) {
    ...
    when {
        line.startsWith("# ") -> Text(
            ...
            style = MaterialTheme.typography.headlineMedium.scaled(textScale),
        )
        line.startsWith("## ") -> Text(
            inlineMarkdown(line.removePrefix("## ")),
            style = MaterialTheme.typography.headlineSmall.scaled(textScale),
        )
        line.startsWith("### ") -> Text(
            inlineMarkdown(line.removePrefix("### ")),
            style = MaterialTheme.typography.titleLarge.scaled(textScale),
        )
        line.startsWith("```") -> Text(
        ...
        line.startsWith("- ") || line.startsWith("* ") -> Text(
            inlineMarkdown("•  ${line.removePrefix("- ").removePrefix("* ")}"),
            style = body,
            modifier = Modifier.padding(start = NeoSpacing.md),
        )
        line.matches(Regex("^\\d+\\. .*")) -> Text(
            inlineMarkdown(line),
            style = body,
            modifier = Modifier.padding(start = NeoSpacing.md),
        )
        line == "---" || line == "***" -> HorizontalDivider(
            Modifier.fillMaxWidth().padding(vertical = NeoSpacing.sm),
        )
...
private fun inlineMarkdown(text: String): AnnotatedString = buildAnnotatedString {
    // Supports common AI Markdown: **bold**, *italic*, __bold__, _italic_,
    // ~~strike~~, `code`, ++underline++ and <u>underline</u>.
    ...
    token.findAll(text).forEach { match ->
        if (match.range.first > cursor) {
            withStyle(currentStyle()) { append(text.substring(cursor, match.range.first)) }
        }
        when (match.value) {
            "**", "__" -> bold = !bold
            "`" -> code = !code
